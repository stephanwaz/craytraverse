...
from craytraverse.crenderer import pyRtrace
import numpy as np
import pytest
...
def test_rtrace_call(tmpdir):
    args = ['-ab', '1', '-ar', '600', '-ad', '2000', '-aa', '0.2', '-as', '1500', '-I+']
...
6.283185e+00    6.283185e+00    6.283185e+00"""
    ...
    check2 = np.einsum('ij,j->i', check, [47.435, 119.93, 11.635])
    # first load
    r = pyRtrace(args + ["-ov", "-n", "1", "sky.oct"])
    ...
    r.set_args(['-pY'])
    ans = r(vecs)
    assert np.allclose(check2, ans[:, 0], atol=.03*179)
#     # #
#     # # reload and change output to float
    r.Cleanup(True)
    r.LoadOctree("sky.oct")

    args2 = args + ['-pY']
    r.set_args(args2)
    #
    test3 = r(vecs).ravel()
    assert np.allclose(check2, test3, atol=.03*179)
#     # #
#     # reload new scene (but keep rendering settings
    r.LoadOctree("sky.oct")
    test3 = r(vecs).ravel()
    assert np.allclose(check2, test3, atol=.03*179)
#     # #
#     # # # change args *  NEW CODE RUNS INTO TROUBLE HERE
# SOME RENDER OPTS AMBIENT SETTINGS ARE NOT GETTING CLEARED, SO THESE
# NEED TO TRIGGER A CLEANUP. NEED TO DEFINE NECESSARY USE CASES AND SIMPLIFY!!! WHAT
# TODO
#     args3 = '-lr 1 -ov -pY -I-'
#     r.Cleanup(False)
#     r.LoadOctree("sky.oct")
#     r.set_args(shlex.split(args3), True)
#
#     test3 = r(vecs).ravel()
#     assert np.allclose([0, 0, 0, 0, np.pi*2*179], test3, atol=.03)
# #     # #
#     # # change back
